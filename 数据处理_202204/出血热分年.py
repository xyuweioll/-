...
b = int(a)
os.chdir(r'C:\Users\xianyu\Desktop\出血热分年')
rootdir = r'C:\Users\xianyu\Desktop\出血热分年'  # 存放文件的根目录
...
for parent, dirnames, filenames in os.walk(rootdir):
    for fn in filenames:
        if os.path.splitext(fn)[1] == '.xlsx':  # 判断文件扩展名是不是为.xlsx  os.path.splitext(“文件路径”)
            ...
            file_op.rename(columns={'发病率': '发病率(1/10万)', '死亡率': '死亡率(1/10万)'}, inplace=True)
            lie_num = file_op.shape[1]  # 获取列数
            hang_num = file_op.shape[0]  # 获取行数
            file_op.drop([hang_num-1], axis=0, inplace=True)  # 删除最后一行
            time_lable = [str(os.path.splitext(fn)[0][0:4])] * (hang_num-1)
