...
# 对numpy进行降版本,跑完程序后 再pip install numpy升级
# import os
...
def NC_to_tiffs(data, value_name, file_name, Output_folder):
    ...
    print(Lon)
    print(Lat)
    u_arr = np.array(nc_data_obj[f'{value_name}'][:])
    # u_arr = np.asarray(nc_data_obj.variables['PM2.5'])  # 这里根据需求输入想要转换的波段名称
    u_arr[u_arr == 65535] = np.nan  # -999    # 设置缺失值为空
    print(type(u_arr))
...
def main():
    ...
    for i in range(len(data_list)):
        ...
        print(File_name)
        NC_to_tiffs(data, Value_name, File_name, Output_folder)
        print(data + '转tif成功')
...
# e.g., PM1, PM2.5, PM10, O3, NO2, SO2, and CO
