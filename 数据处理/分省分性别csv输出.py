...
out_path = r'C:\Users\xianyu\Desktop\output_file.csv'  # 输出文件路径，根据实际情况更改
out_file.to_csv(out_path, index=False, encoding='gbk')
print(f'文件已输出，请至目录{out_path}查看！')
data = pd.read_csv(out_path, low_memory=False)
out_path_excel = out_path[:-4]+'.xlsx'
print('excel文件正在输出中！')
data.to_excel(out_path_excel, index=False)
