...
# GST模块
os.chdir(r'D:\Climate\SELECT-FIX-LL_20220301')   # 指定SELECT文件夹存放的路径，根据实际情况更改
GST_N = '地温(GST)-'
for i in year:
        ...
        file_out.to_csv(path_out, index=False, encoding='gbk')      # 将筛选文件输出到之前创建的文件夹中
print('模块1执行完毕')
# -----------------------------------------------------------------------------
# WIN模块
WIN_N = '风向风速(WIN)-'
for i in year:
        ...
        file_out.to_csv(path_out, index=False, encoding='gbk')
print('模块2执行完毕')
# -----------------------------------------------------------------------------
# PRE模块
PRE_N = '降水(PRE)-'
